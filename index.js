...
const errors = {
  dbOption: {
    ...
    description:
      "MongoDB database {db} option is required, e.g. returned from `MongoClient.connect` method",
  },
  setInterval: {
...
module.exports = class JoSk {
  constructor(opts = {}) {
    ...
    this.collection.createIndex(
      ...
      (indexError) => {
        if (indexError) {
          _debug(
            "[constructor] [collection] [createIndex] [executeAt]",
            indexError
          );
        }
      }
    ...
    this.lockCollection.createIndex(
      { expireAt: 1 },
      { background: false, expireAfterSeconds: 1 },
      (indexError) => {
        if (indexError) {
          _debug(
            "[constructor] [lockCollection] [createIndex] [expireAt]",
            indexError
          );
        }
      }
    ...
    this.lockCollection.createIndex(
      ...
      (indexError) => {
        if (indexError) {
          _debug(
            "[constructor] [lockCollection] [createIndex] [uid]",
            indexError
          );
        }
      }
  ...
  every(name, _delay, data = {}) {
    if (this.__checkState()) {
      return "";
    }

    let uid = this.createUid();
    let delay = typeof _delay === "string" ? humanInterval(_delay) : _delay;
    ...
    this.tasks[uid] = name;
    this.__addTask(uid, true, delay, data);
    return uid;
  }

  delay(name, _delay, data) {
    if (this.__checkState()) {
      return "";
    }
    ...
    this.tasks[uid] = name;
    this.__addTask(uid, false, delay, data);
    return uid;
  }
  ...
  now(name, data) {
    ...
    this.tasks[uid] = name;
    this.__addTask(uid, false, 0, data);
    return uid;
  }
  ...
  __checkState() {
    if (this.isDestroyed) {
      if (this.onError) {
        ...
        _debug(
          "[__checkState] [warn] invoking methods of destroyed JoSk instance, call cause no action"
        );
      }
      return true;
    }
  ...
  __clear(uid) {
    this.collection.deleteOne({ uid }, (deleteError) => {
      this.__errorHandler(
        deleteError,
        "[__clear] [deleteOne] [deleteError]",
        "Error in a callback of .deleteOne() method of .__clear()",
        uid
      );
    });
  ...
  __addTask(uid, isInterval, delay, data = {}) {
    if (this.isDestroyed) {
      return;
    }
    ...
    this.collection.findOne(
      ...
      (findError, task) => {
        if (findError) {
          this.__errorHandler(
            findError,
            "[__addTask] [findOne] [findError]",
            "Error in a callback of .findOne() method of .__addTask()",
            uid
          );
        } else if (!task) {
          this.collection.insertOne(
            ...
            (insertError) => {
              this.__errorHandler(
                insertError,
                "[__addTask] [insertOne] [insertError]",
                "Error in a callback of .insertOne() method of .__addTask()",
                uid
              );
            }
          );
          ...
          if (update) {
            this.collection.updateOne(
              ...
              (updateError) => {
                this.__errorHandler(
                  updateError,
                  "[__addTask] [updateOne] [updateError]",
                  "Error in a callback of .updateOne() method of .__addTask()",
                  uid
                );
              }
            );
  ...
  __execute(task) {
    ...
    const done = (_date) => {
      this.collection.updateOne(
        ...
        (updateError) => {
          this.__errorHandler(
            updateError,
            "[__execute] [done] [updateOne] [updateError]",
            "Error in a callback of .updateOne() method of .__execute()",
            task.uid
          );
        }
      );
    ...
    if (this.tasks && this.tasks[task.uid]) {
      ...
      // Execute task with passed data object
      try {
        this.actions[this.tasks[task.uid]](ready, task.data);
      } catch (err) {}
      ...
      if (lockError) {
        ...
        try {
          ...
          cursor.forEach(
            ...
            (forEachError) => {
              ...
              if (forEachError) {
                ...
                this.__errorHandler(
                  forEachError,
                  "[__runTasks] [forEachError]",
                  "General Error during runtime in forEach endCallback block of __runTasks()",
                  null
                );
              } else if (_ids.length) {
                this.collection.updateMany(
                  ...
                  (updateError) => {
                    if (updateError) {
                      ...
                      this.__errorHandler(
                        updateError,
                        "[__runTasks] [updateMany] [updateError]",
                        "General Error during runtime in updateMany callback block of __runTasks()",
                        null
                      );
                    } else {
          ...
          this.__errorHandler(
            _error,
            "[__runTasks] [catch]",
            "General Error during runtime in try-catch block of __runTasks()",
            null
          );
        }
      }
  ...
  __setNext() {
    ...
    this.nextRevolutionTimeout = setTimeout(
      this.__runTasks.bind(this),
      Math.round(
        Math.random() * this.maxRevolvingDelay + this.minRevolvingDelay
      )
    );
  }
